...
| **F13** | Date/money formatting is inconsistent across several pages. | **DONE 2026-09-19.** Every admin/account/learner page that hand-rolled money, counts or dates now calls `lib/format.ts` (new `formatMoney(..., {exact})`, `formatMoneyList`, `formatNumber`, `formatTime`, `formatTimestamp`; 3 duplicate local helpers deleted, browser-locale `toLocale*()` calls gone). An ESLint `no-restricted-syntax` rule now fails the gate on any raw `toLocale*` call under `app/` or `components/`, so it cannot drift back. Unit tests pin the convention. Finance/invoice tables now consistently show 2 decimals. |
| **F17** | Screenshot-pass UX defects: push prompt placement, mobile header/catalogue/player and learner IA. | **DONE 2026-09-19.** **F17a done** — push opt-in no longer fetches/prompts on `/checkout/*` or the lesson player (`/learn/<enrolment id>`); vitest route-gating tests. **F17b done** — public header collapses behind an `aria-expanded` toggle under 760px (was an overflowing scroll strip); vitest ARIA tests plus Playwright mobile-viewport (375px) no-overflow and axe checks on every public page. **F17d done** — `.foot-nav` stacks full-width under 560px so "Next lesson 🔒" stays in the content column (CSS-only; no player e2e fixture exists, so no automated coverage). **F17c done** — catalogue filters collapse behind an `aria-expanded` "Filters (N active)" toggle under 900px, always shown above it; vitest covers the toggle (the layout itself has no e2e check: the catalogue only renders its browser when the API returns courses, and the `web` e2e job has no API). **F17e done** — the dashboard's "Resume" continue CTA already existed and is fed by `next_lesson`; the gap was the nav: a signed-in learner's header was one flat row of 7–8 equal links. Main nav is now the five learning destinations; Subscription/Invoices (and Admin, staff only) move to a quieter labelled "Account" group beside the avatar; vitest covers the split. **F17a–e all done.** |
| **F18** | Anonymous storefront pages are force-dynamic/no-store and lack route loading states. | **DONE 2026-09-19.** Storefront and theme reads now `revalidate` every 60 s instead of `no-store`, and the tenant is part of the cache key (`_tenant=<host>` on the URL, ignored by the API — pinned by an API test) because Next's data cache is keyed by URL while the tenant otherwise travels only in the `X-Tenant-Host` header. Pages themselves stay dynamic on purpose: they read `headers()` and their HTML depends on the Host, so a route-level cache shared across tenants would be a leak. Ten storefront routes gained a `loading.tsx` (shared accessible `RouteLoading`), with a vitest guard that fails if one goes missing. Not covered end to end: the public e2e job has no API, so the cache-hit behaviour against a live API is unit-tested only. |
...
| Storefront delivery | F18 caching/revalidation and `loading.tsx` coverage. | **DONE 2026-09-19.** |
