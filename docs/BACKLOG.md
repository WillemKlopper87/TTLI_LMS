...
| **P3** | **PART DONE 2026-08-21 — users & roles shipped; branding and domains still open.** Done: `GET /tenant/roles`, `GET/POST /tenant/users`, role grant/revoke, suspend/reinstate, `/admin/people`, with a no-privilege-escalation rule (you may only grant a role whose permissions you already hold) and a no-self-change rule (revoking your own last `tenant:manage` would lock the tenant out). `rbac.role.assigned`/`revoked` are finally emitted — they had existed unused since 0001. **Still open:** `PATCH /tenant/theme` + logo upload (#44) and `GET/POST/DELETE /tenant/domains` (#45); both still change only by migration | M | The severe half is closed — a tenant can now onboard its own staff without a developer | PART DONE |
...
5. **P3** (branding + domains remain) → **P4** — SSO. **← next**
