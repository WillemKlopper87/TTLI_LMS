...
| **P5** | **Learning paths** (Pass E; audit #7). `learning_paths` + `learning_path_courses`, path entitlement / `Product.kind="path"`, progress rollup, admin builder, learner page, path certificate. **Phase 1/4 done 2026-08-23**: schema (`0035` — `learning_paths`/`learning_path_courses`/`learning_path_tenant_assignments`/`path_enrolments`, `products.learning_path_id`, `certificates.enrolment_id` now nullable with a `path_enrolment_id` sibling + CHECK) plus full admin authoring behind `/admin/paths` (create/edit/reorder-by-drag/readiness/publish/tenant-assign), 3 API tests, axe-clean, live-smoked end to end through the real UI. **Still open**: Phase 2 commerce (`Product.kind="path"` fulfilment branch, `path_enrolments` creation, public catalogue), Phase 3 progress rollup + certificate issuance, Phase 4 learner-facing browse/buy/dashboard | L | Core LMS vocabulary and a Professional-tier ✅ in the feature matrix. `learning_path` returned **zero hits** in the codebase before this pass | IN PROGRESS |
