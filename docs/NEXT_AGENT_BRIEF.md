...
- **No web tests** (~~no lint~~ — ESLint gate since 2026-08-20). Remaining minimum: Playwright smoke on login → learn → admin, + axe in that same run (also closes Phase 4.5's last item).
...
| **Frontend quality gates** | ESLint gate added 2026-08-20 (`core-web-vitals`, errors blocking; `set-state-in-effect` warn-only for 34 legacy sites — cleanup list). Still open: unit/e2e tests (Playwright smoke), axe gate, Lighthouse | Lint done; tests and axe remain |
