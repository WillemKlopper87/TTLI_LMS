> **Archived 2026-09-11.** Historical snapshot of the next-agent brief as of 2026-09-08. Superseded by [`docs/NEXT_AGENT_BRIEF.md`](../NEXT_AGENT_BRIEF.md), which now carries current state only. Retained for the engineering rationale in §4 and §7; every status claim below is frozen at its date.
...
| 4 | ~~**SSO — Entra ID / OIDC**~~ (#46; Pass D) | **DONE.** Per-tenant OIDC shipped 2026-08-22 (`BACKLOG.md` P4); the browser half — the callback page and the sign-in button, without which a configured tenant reached a 404 — landed 2026-09-05 (`REMEDIATION_LEDGER.md` H-15). SAML is still later scope | Standard corporate procurement gate for the Team/Corporate tiers | DONE |
