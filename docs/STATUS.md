...
- [ ] `clamav/clamav-debian:stable` — still open. Not folded into the dependency-upgrade sprint (A–G above), which covered every other flagged image/package but never scoped this one; not urgent (reachable only from the local dev network, on a non-`:latest` tag), but a real gap, not silently dropped
