...
**Updated:** 2026-08-10 (Phase 5 sprint 2 — manager visibility, REQ-TEN-03's demo target — built and verified)
...
| 5 | Corporate, workshops, marketing | Sprint 1: organisations, seat-pool entitlements, PO checkout (closes Phase 3's deferred PO gap too). Sprint 2: manager visibility (REQ-TEN-03) — the demo target itself, met. Sprints 3–4 (workshops, CRM/marketing) not started | ~30% |
| 6 | AI insights | Not started | 0% |
| 7 | Hardening and cloud | Not started | 0% |

| Gate | Status |
|---|---|
| `ruff check` / `ruff format --check` | **PASS** — 134 files |
| `mypy src` (strict) | **PASS** — 95 source files |
| `pytest` | **PASS** — 173 passed, **0 skipped**, run twice for determinism (against real Postgres, Redis, MinIO, Mailpit, ClamAV *and* real ffmpeg) |
...
| `alembic upgrade head` | **PASS** — at `0017` |
...
| CI (`.github/workflows/api.yml`), `quality` + `web` jobs | **PASS** — green on both jobs on the first try, [run 31405256087](https://github.com/WillemKlopper87/TTLI_LMS/actions/runs/31405256087) (quality 3m43s, web 1m1s) — includes the migration round-trip check passing in a clean environment, confirming the `0016` downgrade fix |

**Headline:** 173 tests (0 skipped), 73 endpoints, 54 tables (events partitioned monthly ×14), 17 migrations, typed TS client with a CI drift gate, email delivery through the arq worker with retries, 22 `apps/web` routes, CSP + security headers on every `apps/web` response, virus-scanned uploads, dependency scanning (`pip-audit`, `npm audit`) wired into CI, a server-side completion rule engine gating real course progress across video/quiz/survey/assignment, a real ported VOD transcode pipeline with signed HLS playback and WebVTT captions, quizzes/surveys/assignments with real auto-grading and anonymous-survey pseudonymisation, certificates/badges with a real reportlab-rendered PDF and encrypted+blind-indexed public verification, a printable transcript, an installable PWA with a real offline shell, a WCAG 2.1 AA-verified colour system, a real organisation seat-pool purchasing flow (PO checkout → finance approval → seat pool → bulk invite/CSV import → revoke-and-reassign), and manager visibility gated on all three of REQ-TEN-03's conditions at once (course toggle, tenant toggle, a real per-organisation grant).
