...
def _block_response(block: LessonBlock) -> LessonBlockResponse:
    return LessonBlockResponse(
        ...
        body=block.body,
        video_asset_id=str(block.video_asset_id) if block.video_asset_id else None,
        audio_asset_id=str(block.audio_asset_id) if block.audio_asset_id else None,
        quiz_id=str(block.quiz_id) if block.quiz_id else None,
        survey_id=str(block.survey_id) if block.survey_id else None,
        assignment_id=str(block.assignment_id) if block.assignment_id else None,
        completion_rules=block.completion_rules,
    )


async def _lesson_response(session: AsyncSession, lesson: Lesson) -> LessonResponse:
...
async def list_courses(principal: PrincipalDep, session: SessionDep) -> CoursesPageResponse:
    ...
    courses = await courses_service.list_courses(session, tenant_id=principal.tenant_id)
...
async def publish_course(
    ...
    course = await courses_service.publish_course(
        session, course_id=_parse_uuid(course_id), tenant_id=principal.tenant_id
    )
...
async def unpublish_course(
    ...
    principal.require("course:publish")
    course = await courses_service.unpublish_course(
        session, course_id=_parse_uuid(course_id), tenant_id=principal.tenant_id
    )
    await audit.record(
        session,
...
async def list_modules(
    ...
    principal.require("course:view")
    modules = await courses_service.list_modules(
        session, course_id=_parse_uuid(course_id), tenant_id=principal.tenant_id
    )
...
async def list_lessons(
    ...
    principal.require("course:view")
    lessons = await courses_service.list_lessons(
        session, module_id=_parse_uuid(module_id), tenant_id=principal.tenant_id
    )
    return LessonsPageResponse(
        items=[await _lesson_response(session, lesson) for lesson in lessons]
    )
    ...
    for module_outline in outline:
        lessons = [
            PublicLessonRow(
                ...
                blocks=[
                    PublicBlockRow(
                        id=str(b.block.id), position=b.block.position, block_type=b.block.block_type
                    )
...
async def list_public_courses(session: SessionDep, tenant: TenantDep) -> PublicCoursesResponse:
    ...
    for course in courses:
        outline = await wizard_service.get_outline(
            session, course_id=course.id, tenant_id=tenant.id
        )
...
async def get_public_curriculum(
    ...
    outline = await wizard_service.get_outline(session, course_id=course.id, tenant_id=tenant.id)
    modules = _public_modules(outline)
    prices = await courses_service.public_prices_for_courses(
