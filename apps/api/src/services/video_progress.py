...
async def record_heartbeat(
    ...
    *,
    ...
    stmt = select(VideoProgress).where(
        VideoProgress.enrolment_id == enrolment_id, VideoProgress.lesson_id == lesson_id
    )
    ...
    if progress is None:
        progress = VideoProgress(
            id=uuid7(), tenant_id=tenant_id, enrolment_id=enrolment_id, lesson_id=lesson_id
        )
    ...
    if progress.last_heartbeat_at is not None:
        ...
        elapsed = max(Decimal("0"), min(raw_elapsed, HEARTBEAT_MAX_INTERVAL_SECONDS))
...
async def watch_percentage(
    session: AsyncSession, *, enrolment_id: uuid.UUID, lesson_id: uuid.UUID, duration_seconds: int
) -> float | None:
